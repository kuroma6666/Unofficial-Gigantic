 ...
 * @author kuroma6666
 */
object DailyRankingMenu : BookMenu() {
    ...
    init {
        registerButton(size - 9, BackButton(this, DailyRankingSelectMenu))
    ...
    override fun setItem(inventory: Inventory, player: Player, page: Int): Inventory {
        ...
        player.getOrPut(Keys.MENU_RANKING_PLAYER_LIST)
                .forEachIndexed { index, rankingPlayer ->
                    inventory.setItemAsync(player, index + offset, RankingButtons.DAILY_RANKING_PLAYER(rankingPlayer, score))
                }
    ...
    override fun getButton(player: Player, page: Int, slot: Int): Button? {
        ...
        return getButtonMap()[slot]
                ?: RankingButtons.DAILY_RANKING_PLAYER(player.getOrPut(Keys.MENU_RANKING_PLAYER_LIST)[index], score)
    }
